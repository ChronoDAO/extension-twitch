...
import { getCosmeticsWebsite } from "../../../../utils/twitch";
...
export default function NFT({ NFT, ambassadorCode }) {
  ...
  const [cosmeticsWebsite, setCosmeticsWebsite] = useState("");

  if (cosmeticsWebsite === "") {
    cosmeticsWebsite && setCosmeticsWebsite("getCosmeticsWebsite()");

    // useEffect(() => {
    //   const fetchData = async () => {
    //     const result = await getCosmeticsWebsite();
    //     setCosmeticsWebsite(result);
    //   }

    //   fetchData();
    //   !cosmeticsWebsite && setCosmeticsWebsite("https://cosmetics.chronodao.com/")
    ...
    return (
      <div>
        <div>
          <a
            key={NFT.id}
            href={`https://cosmetics.chronodao.com/items/BT0/${NFT.optionName}?utm_source=ambassador&utm_campaign=${ambassadorCode}`}
            target="_blank"
          >
            <img
              src={NFT.imageUrl}
